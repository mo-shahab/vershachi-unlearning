...
class GammaUnknownShape:
    def __init__(self, dim, nparam, params, prior_params):
        ...
        self.log_prior_weight = tf.cast(
            tf.log(prior_params["weight"].reshape(-1, 1)), dtype=tf.float64
        )
        # (len(self.prior), 1)
    ...
    def log_likelihood(self, theta, data, sum_over_data=True):
        ...
        likelihood = tfp.distributions.Gamma(
            ...
            rate=tf.expand_dims(
                tf.tile(
                    tf.expand_dims(tf.exp(self.params["log_rate"]), axis=0),
                    multiples=(nsample, 1),
                ),
                axis=0,
            ),
...
class UCILogisticRegression:
    def __init__(self, dataset, order, prior_gaussian_mean=0, prior_gaussian_std=10000):
        ...
        if self.order == 1:
            ...
            self.nparam_per_class = int(
                (self.dim - 1) * (self.dim - 2) / 2 + 2 * (self.dim - 1) + 1
            )
            self.nparam = int(self.nparam_per_class * (self.nclass - 1))
    ...
    def log_likelihood(self, theta, data, sum_over_data=True):
        ...
        if self.order == 1:
            ...
            x = tf.concat(
                [x, x2, inter_x, tf.ones((ndata, 1), dtype=tf.float64)], axis=1
            )
        ...
        for i in range(self.nclass - 1):
            theta_i = tf.gather(
                theta,
                indices=list(
                    range(i * self.nparam_per_class, (i + 1) * self.nparam_per_class)
                ),
                axis=1,
            )
    ...
    def sample_data_np(self, size, xbound=None, seed=0):
        ...
        """
        print(
            "sample_data_np: Reading from dataset: {}. Ignore xbound".format(
                self.dataset
            )
        )
...
class MoonClassification:
    ...
    def __init__(
        ...
        self.xu, _ = skdata.make_moons(
            n_samples=nu, shuffle=True, noise=noise_std, random_state=0
        )
        # xu: (nu,2)
        ...
        if log_lengthscales is None or log_sigma is None:
            ...
            self.log_lengthscales = tf.Variable(
                np.zeros(self.dim - 1), dtype=tf.float64
            )
        ...
        self.K = gp_utils.computeKmm(
            self.xu, self.lengthscales, self.sigma, dtype=tf.float64
        )
        self.KInv = gp_utils.chol2inv(self.K)

        self.prior = tfp.distributions.MultivariateNormalFullCovariance(
            loc=tf.zeros(self.nu, dtype=tf.float64), covariance_matrix=self.K
        )
    ...
    def log_likelihood(self, theta, data, sum_over_data=True):
        ...
        logprobs = (
            predicted_f * y - half_pred_f - tf.reduce_logsumexp(normalizer, axis=0)
        )
        # (nsample,ndata)
    ...
    def sample_data_np(self, size, noise_std=0.2, xbound=None, seed=0):
        X, labels = skdata.make_moons(
            n_samples=size, shuffle=True, noise=noise_std, random_state=seed
        )
        # X.shape = (n_samples,2)
