...
parser = argparse.ArgumentParser(
    description="Plot the data, posterior mean and variance of the synthetic moon classification dataset."
)

parser.add_argument(
    "--exper",
    help="in {moon, moon_random, moon_rm_30, moon_rm_40, moon_rm_50}",
    required=False,
    type=str,
    default="moon",
)
...
posterior_lprobs = experiment_data["model"].log_posterior_predictive(
    param_samples, padding_dummy_output(x).astype(np.float64)
)
...
def get_posterior_lprobs(learned_param):
    with tf.compat.v1.Session() as sess:
        ...
        for _ in range(n_iter):
            ...
            posterior_lprobs_i = scipy.special.logsumexp(
                posterior_lprobs_i, axis=2, keepdims=True
            ) - np.log(posterior_lprobs_i.shape[2])
            # (nclass, ndata, 1)

            posterior_lprobs_np.append(posterior_lprobs_i)

        posterior_lprobs_np = np.concatenate(
            posterior_lprobs_np, axis=2
        )  # concat over the sample dimension
        # (nclass, ndata, n_param_samples)

        posterior_lprobs_np = scipy.special.logsumexp(
            posterior_lprobs_np, axis=2
        ) - np.log(posterior_lprobs_np.shape[2])
...
for percentage in selected_percentages:
    elbo_params = pickle.load(
        open(
            "{}/data_remain_data_by_unlearn_elbo_{}.p".format(prefix, percentage), "rb"
        )
    )
    elbo_probs[percentage] = np.exp(get_posterior_lprobs(elbo_params))

    eubo_params = pickle.load(
        open(
            "{}/data_remain_data_by_unlearn_eubo_{}.p".format(prefix, percentage), "rb"
        )
    )
...
hm = seaborn.heatmap(
    np.flipud(full_probs[1, :].reshape(n, n)),
    vmin=0,
    vmax=1,
    cmap="YlGnBu",
    ax=axs[0, 0],
...
hm = seaborn.heatmap(
    np.flipud(remain_probs[1, :].reshape(n, n)),
    vmin=0,
    vmax=1,
    cmap="YlGnBu",
    ax=axs[0, 1],
...
for percentage in selected_percentages:
    ...
    hm = seaborn.heatmap(
        np.flipud(elbo_probs[percentage][1, :].reshape(n, n)),
        vmin=0,
        vmax=1,
        cmap="YlGnBu",
        ax=axs[int(plot_idx / 4), plot_idx % 4],
        cbar=show_cbar,
    )
    hm.set(xticks=list(range(0, 50, 16)))
    hm.set(xticklabels=["{:.1f}".format(i) for i in x1d[list(range(0, 50, 16))]])
    hm.set(yticks=list(range(0, 50, 16)))
    hm.set(
        yticklabels=["{:.1f}".format(i) for i in x1d[list(reversed(range(0, 50, 16)))]]
    )

    axs[int(plot_idx / 4), plot_idx % 4].set_title(
        r"$\tilde{q}_v(y_x|\mathcal{D}_r;$" + "$\lambda$={}".format(percentage) + r"$)$"
    )

    plot_idx += 1

    hm = seaborn.heatmap(
        np.flipud(eubo_probs[percentage][1, :].reshape(n, n)),
        vmin=0,
        vmax=1,
        cmap="YlGnBu",
        ax=axs[int(plot_idx / 4), plot_idx % 4],
    ...
    axs[int(plot_idx / 4), plot_idx % 4].set_title(
        r"$\tilde{q}_u(y_x|\mathcal{D}_r;$" + "$\lambda$={}".format(percentage) + r"$)$"
    )
