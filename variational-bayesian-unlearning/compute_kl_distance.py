...
parser = argparse.ArgumentParser(
    description="Compute KL distance between approximate predictive distributions."
)

parser.add_argument(
    "--folder",
    help="location to read the training result",
    required=False,
    type=str,
    default="result",
)
...
parser.add_argument(
    "--outfolder",
    help="location to write the data",
    required=False,
    type=str,
    default="plot_data",
)
...
parser.add_argument(
    "--exper",
    help="in {moon, moon_random, moon_rm_30, moon_rm_40, moon_rm_50, banknote_authentication1}",
    required=False,
    type=str,
    default="moon",
)
...
parser.add_argument(
    "--appr",
    help="in {gauss_fullcov, gauss_diag, maf}",
    required=False,
    type=str,
...
def get_likelihoods(learned_param):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        removed_likelihoods_np = np.concatenate(
...
print("Likelihood of full-data posterior")
sys.stdout.flush()
...
print("Likelihood of re-trained posterior")
sys.stdout.flush()
retrain_learned_param = pickle.load(
    open(
        "{}/{}/{}/remain_data_retrain_post.p".format(folder, experiment, approximate),
        "rb",
    )
)
...
for i, percentage in enumerate(elbo_mode_percentages):
    print("Likelihood of ELBO unlearned posterior {}".format(percentage))
    sys.stdout.flush()
...
for i, percentage in enumerate(eubo_mode_percentages):
    print("Likelihood of EUBO unlearned posterior {}".format(percentage))
    sys.stdout.flush()
    learned_param = pickle.load(
        open(
...
try:
    ...
    with open(outfilename, "wb"):
        pass  # No need to do anything here; the file is created
    print(f"{outfilename} created successfully.")
...
try:
    # Open the file in binary write mode ("wb") for pickling
