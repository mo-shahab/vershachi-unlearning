...
def chol2inv(mat, dtype=tf.float64):
    ...
    invlower = tf.linalg.solve(tf.linalg.cholesky(mat), tf.eye(n, dtype=dtype))
...
def compute_mean_f(x, Xsamples, Fsamples, l, sigma, KInv=None, dtype=tf.float64):
    """
    l: 1 x d
    Fsamples: k x m
    Xsamples: m x d
    ...
    mean = tf.expand_dims(kstar, axis=0) @ (
        tf.expand_dims(KInv, axis=0) @ tf.expand_dims(Fsamples, axis=2)
    )
    # (k, n, 1)
...
def compute_mean_var_f_marginalize_u(
    x, Xsamples, l, sigma, mean_fsamples, cov_fsamples, KInv=None, dtype=tf.float64
):
    """
    l: 1 x d
    Xsamples: m x d
    ...
    mean = kstar @ (KInv @ tf.reshape(mean_fsamples, shape=(m, 1)))
    # (n,1)
