...
def load_data(file_paths):
    data = np.concatenate([load_npz(file_path).toarray() for file_path in file_paths]).astype(int)
    return data
...
def save_split_data(X_train, X_test, y_train, y_test, num_clusters=2, prefix='purchase', output_dir=None):
    if output_dir is None:
        output_dir = os.getcwd()  # Use the current working directory if output_dir is not specified
