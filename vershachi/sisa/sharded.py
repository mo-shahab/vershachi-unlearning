...
def fetchShardBatch(container, label, shard, batch_size, dataset, offset=0, until=None):
    ...
    spec = importlib.util.spec_from_file_location(datasetfile['dataloader'], dataloader_path)
...
def fetchTestBatch(dataset, batch_size):
    ...
    spec = importlib.util.spec_from_file_location(datasetfile['dataloader'], dataset_module_path)
    dataloader_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(dataloader_module)
    ...
    while limit <= datasetfile['nb_test'] - batch_size:
        limit += batch_size
        yield dataloader_module.load(np.arange(limit - batch_size, limit), category='test')
    if limit < datasetfile['nb_test']:
        yield dataloader_module.load(np.arange(limit, datasetfile['nb_test']), category='test')
