...
def FL_Train(init_global_model, client_data_loaders, test_loader, FL_params):
    ...
    #         #IMPORTANTï¼šè¿™é‡Œæœ‰ä¸€ç‚¹è¦æ³¨æ„ï¼Œå°±æ˜¯global_train_onceåœ¨è®­ç»ƒè¿‡ç¨‹ä¸­ï¼Œæ˜¯ç›´æŽ¥åœ¨inputçš„client_modelsä¸Šè¿›è¡Œè®­ç»ƒï¼Œå› æ­¤outputçš„client_modelsä¸Žinputçš„client_modelsæ˜¯åŒä¸€ç»„æ¨¡åž‹ï¼Œåªä¸è¿‡inputæ²¡æœ‰ç»è¿‡è®­ç»ƒï¼Œè€Œoutputç»è¿‡äº†è®­ç»ƒã€‚
    # #   IMPORTANTï¼šå› æ­¤ï¼Œä¸ºäº†å®žçŽ°Federated unlearningï¼Œæˆ‘ä»¬éœ€è¦åœ¨global trainä¹‹å‰å°±å°†clientâ€”â€”modelsä¸­çš„æ¨¡åž‹è¿›è¡Œä¿å­˜ã€‚å¯ä»¥ä½¿ç”¨deepcopyï¼Œæˆ–è€…ç¡¬ç›˜ioæ–¹å¼ã€‚
...
def FL_Retrain(init_global_model, client_data_loaders, test_loader, FL_params):
    ...
    for epoch in range(FL_params.global_epoch):
        ...
        the output is trained.
        #Therefore, in order to implement Federated Unlearning, we need to save the models in Client -- Models before global Train.
        You can use DeepCopy, or hard disk IO.
        """
        global_model = fedavg(client_models)
        # print(30*'^')
