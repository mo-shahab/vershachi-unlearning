...
def unlearning(old_GMs, old_CMs, client_data_loaders, test_loader, FL_params):
    ...
    (ref_client_models - new_global_model) / ||ref_client_models - new_global_model||ï¼ŒIndicates the direction of model parameter iteration starting with a new global model that removes a user.Mark the direction as step_direction

    ||old_client_models - old_global_model||ï¼ŒIndicates the step size of the model parameter iteration starting with the old global model with a user removed.Step step_length

    So, the final direction of the new reference model is step_direction*step_length + new_global_models
    """
    """
    ...
    for unblearning FLï¼šnewGM_t-->newCM0, newCM1, newCM2, newCM3--> newGM_t+1
...
def unlearning_without_cali(old_global_models, old_client_models, FL_params):
    ...
    """
    """
    The basic process is as followsï¼šFor unforgotten FL:oldGM_t--> oldCM0, oldCM1, oldCM2, oldCM3--> oldGM_t+1
