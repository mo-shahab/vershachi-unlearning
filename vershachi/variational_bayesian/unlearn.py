...
class Unlearnizable:
    ...
    def __init__(
        ...
        self.posterior_dist = approximate_dist_class(
            self.nparam, **approximate_dist_config
        )

        self.data_plc = tf.compat.v1.placeholder(
            shape=(None, self.xdim), dtype=tf.float64
        )
        ...
        self.learn_op = tf.compat.v1.train.AdamOptimizer().minimize(
            -self.elbo, var_list=list(self.posterior_dist.param.values())
        )

        self.full_data_posterior_dist = approximate_dist_class(
            self.nparam, **approximate_dist_config
        )
        ...
        log_likelihood = tf.where(
            tf.math.greater(
                full_data_posterior_logprob_theta_samples, self.log_threshold
            ),
        ...
        # - self.full_data_posterior_dist.log_prob(post_theta_samples))
        self.unlearn_op = tf.compat.v1.train.AdamOptimizer().minimize(
            self.eubo, var_list=list(self.posterior_dist.param.values())
        )

        full_post_theta_samples = self.full_data_posterior_dist.sample(nsample)
        # (nsample, nparam)

        full_data_post_logprob_full_theta_samples = (
            self.full_data_posterior_dist.log_prob(full_post_theta_samples)
        )

        log_likelihood_removed_data = self.log_likelihood_func(
            full_post_theta_samples, self.data_plc
        )
        # (nsample,)
        ...
        log_likelihood_removed_data = tf.where(
            tf.math.greater(
                full_data_post_logprob_full_theta_samples, self.log_threshold
            ),
        ...
        self.unlearn_elbo_op = tf.compat.v1.train.AdamOptimizer().minimize(
            -self.unlearn_elbo, var_list=list(self.posterior_dist.param.values())
        )
    ...
    def learn(self, data, init_post_param=None, ntrain=1000, batchsize=1000000):
        ...
        for i in range(ntrain):
            self.sess.run(
                self.learn_op,
                feed_dict={
                    self.data_plc: data[idxs[nstart : (nstart + batchsize)], ...]
                },
            )
            nstart += batchsize
            ...
            if i % 5000 == 0:
                print(
                    "{}. {:.4f} in {:.4f}s".format(
                        i,
                        self.sess.run(
                            self.unlearn_elbo, feed_dict={self.data_plc: data}
                        ),
                        time.time() - start_time,
                    )
                )
