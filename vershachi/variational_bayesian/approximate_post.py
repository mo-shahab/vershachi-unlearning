...
class Gaussian:
    ...
    def __init__(self, dim):
        ...
        self.param = {
            "loc": tf.Variable(tf.zeros(self.dim, dtype=tf.float64), dtype=tf.float64),
            "sqrt_cov": tf.Variable(
                tf.eye(self.dim, dtype=tf.float64), dtype=tf.float64
            ),
        }
        sqrt_cov = tf.linalg.band_part(self.param["sqrt_cov"], -1, 0)

        self.cov = sqrt_cov @ tf.transpose(sqrt_cov)

        self.dist = tfp.distributions.MultivariateNormalFullCovariance(
            loc=self.param["loc"], covariance_matrix=self.cov
        )
    ...
    def find_mode(self, param=None, n_init=10, ntrain=100):
        mode_log_prob = self.log_prob(
            tf.reshape(self.param["loc"], shape=(1, self.dim))
        )
    ...
    def __init__(self, dim):
        ...
        self.param = {
            ...
            "log_scale": tf.Variable(
                tf.ones(self.dim, dtype=tf.float64), dtype=tf.float64
            ),
        }
        scale = tf.exp(self.param["log_scale"])

        self.dist = tfp.distributions.MultivariateNormalDiag(
            loc=self.param["loc"], scale_diag=scale
        )
...
class MAF:
    ...
    def __init__(self, dim, nbijectors=5, hidden_layers=[5, 5]):
        ...
        self.maf.sample(
            1
        )  # dummy, so that tf.trainable_variables() does not return empty list
    ...
    def find_mode(self, param=None, n_init=10, ntrain=100):
        ...
        initializers = tf.Variable(
            tf.zeros_like(samples), dtype=tf.float64, name="initializers"
        )
        # (n_init, self.dim)
