...
def get_experiment(experiment, config={}):
    ...
    if experiment == "gamma_unknown_shape":
        ...
        prior_params = {
            ...
            "dist": [
                {"loc": np.ones(nparam) * 0.0, "cov": np.eye(nparam)},
                {"loc": np.ones(nparam) * 2.0, "cov": np.eye(nparam) * 4.0},
            ],
        }

        model = GammaUnknownShape(
            dim,
            nparam,
            params={"log_shape": np.ones(dim) * 1.0, "log_rate": np.ones(dim) * 1.0},
        ...
        remain_idxs = np.array(
            list(set(range(config["ndata"])).difference(set(config["removed_idxs"])))
        )

        ndata = config["ndata"]
        data = model.sample_data_np(
            size=ndata, xbound=[config["xmin"], config["xmax"]], seed=config["seed"]
        )
        ...
        remain_idxs = np.array(
            list(set(range(config["ndata"])).difference(set(removed_idxs)))
        )
        ...
        model = UCILogisticRegression(
            dataset="banknote", order=1, prior_gaussian_mean=0, prior_gaussian_std=100
        )
