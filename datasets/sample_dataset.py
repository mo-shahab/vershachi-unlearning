...
def create_subset(data, subset_size):
    ...
    subset_indices_data = np.concatenate(
        [
            np.arange(start, end)
            for start, end in zip(subset_indptr[:-1], subset_indptr[1:])
        ]
    )
    ...
    subset_data = {
        ...
        "indptr": np.cumsum(
            [0]
            + [end - start for start, end in zip(subset_indptr[:-1], subset_indptr[1:])]
        ),
        "format": data["format"],
        "shape": (
            subset_size,
            data["shape"][1],
        ),  # Assuming shape[0] is the number of rows
...
def main():
    ...
    output_files = [
        "subset_data1.npz",
        "subset_data2.npz",
    ]  # Update with desired output file paths
